...
function loadedNumDone() {
    return Object.entries(loaded).filter(([k, {
        status
    }]) => status === LOADED_DONE).length
}
...
L.Util.setOptions(bookmarkPopup, {
    autoPanPadding: [0, 180]
})
...
function downloadRect(tileId) {
    ...
    var feedbackTooltip = L.tooltip(L.latLng(center), {
        content: '<img src="assets/images/loader.gif">'
    })
...
const openBookmarkInApp = (() => {
    const {
        lat,
        lng
    } = bookmarkPopup.options.bookmark.latlng
...
function updateDownloadStatuses() {
    ...
    let anyLocallyTriggeredDeletions = Object.values(loaded).filter(({
        deleting
    }) => deleting).length > 0
            ...
            Object.keys(downloadRects).forEach(tileId => {
                ...
                if (tooltipContent) {
                    if (tooltip) {
                        ...
                        downloadRects[tileId].bindTooltip(tooltipContent, {
                            permanent: true
                        })
                    }
                } else {
...
function setGeoJsonOpacityAndBackground() {
    if (map.getZoom() > 6) {
        $('.leaflet-container').css({
            background: uiStyle.background.zoomedIn.color
        })
    } else {
        $('.leaflet-container').css({
            background: uiStyle.background.water.color
        })
    }
