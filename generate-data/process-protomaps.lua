...
-- See https://github.com/geofabrik/shortbread-tilemaker/ hash 68c28e503f0281af82645bf871c45fca329e77ed
...
function process_place_layer(node)
	...
	-- From looking at protomaps.js, it seems like it supports "country"
	-- "state", "city" and "neighborhood". We will
	-- collapse "town", "village", "suburb", "hamlet" "island" and "locality"
    -- into pmap:kind="city". (Shortbread classifies things in a much more
    -- granular way separately.)
	--
	-- TODO "locality" is by definition an abandoned hamlet/town/etc. I
	-- wanted it to show up on the map *somehow*. Perhaps there's a better
	-- way to make it something a historical marker in protomaps.

	if place == "country" then
		mz = 3
        kind = "country"
        -- Shortbread did not support this
	...
	elseif place == "locality" or place == "island" then
        ...
        kind = "neighbourhood" -- Best guess. Even this may be too much.
	elseif  place == "isolated_dwelling" or place == "farm"  then
		mz = 10
		if population == "" then
			population = "5"
		end
        kind = "neighbourhood" -- This might be too much.
	end
...
function process_water_polygons(way, way_area)
	...
	local mz = inf_zoom
	local kind = ""
	local is_river = (natural == "water" and water == "river") or waterway == "riverbank" or waterway == "river"

    -- TODO - likely axe this? other than the glacier part?
	...
	--if mz < inf_zoom then
		...
		way:MinZoom(5) -- way:MinZoom(mz)
		-- way:Attribute("kind", kind)
		-- way:AttributeNumeric("way_area", way_area)
        -- way:ZOrder(math.floor(math.log(way_area)))
		if way:Holds("name") then
			-- way:LayerAsCentroid("water")
			-- way:MinZoom(14)
			-- way:Attribute("kind", kind)
			-- way:AttributeNumeric("way_area", way_area)
            -- way:ZOrder(math.floor(math.log(way_area)))
...
-- Redoing this function quite a bit for protomaps
-- Also subsuming part of the former function `process_sites`
function process_land(way)
    ...
    elseif natural == "scrub" or natural == "heath" then
        do_natural(way, "wood", 11)
	elseif natural == "sand" or natural == "beach" then
        do_natural(way, "sand", 10)
	elseif landuse == "residential" then
        do_landuse_landuse(way, "residential", 10)
	elseif landuse == "cemetery" or amenity == "grave_yard" then
        do_landuse_landuse(way, "cemetery", 13)
	elseif landuse == "industrial" or landuse == "landfill" then
        do_landuse_landuse(way, "industrial", 10)
	elseif landuse == "grass" or natural == "grassland" then
        do_landuse_landuse(way, "grass", 11)
    elseif amenity == "school" or amenity == "kindergarten" or amenity == "university" or amenity == "college" or amenity == "hospital" then
        do_landuse_amenity(way, amenity, 14)
	elseif leisure == "golf_course" or leisure == "park" or leisure == "garden" or leisure == "playground" or leisure == "miniature_golf" or landuse == "orchard" or landuse == "vineyard" then
        do_landuse_amenity(way, "park", 11)
	else
        -- TODO - this covers water too. this sucks.
        -- TODO - Maybe I need the urban shapefiles etc.
        do_earth(way, 10)
    end
end

function do_earth(way, mz)
	way:Layer("earth", true)
	way:MinZoom(mz)
end

function do_natural(way, natural, mz)
	way:Layer("natural", true)
	way:Attribute("natural", natural)
	way:MinZoom(mz)
end

function do_landuse_landuse(way, landuse, mz)
	way:Layer("earth", true)
	way:Attribute("landuse", landuse)
	way:MinZoom(mz)
end

function do_landuse_amenity(way, amenity, mz)
	way:Layer("earth", true)
	way:Attribute("amenity", amenity)
	way:MinZoom(mz)
end

function do_landuse_leisure(way, leisure, mz)
	way:Layer("earth", true)
...
function process_streets(way)
	...
	if mz <= 13 then
		way:Layer("roads_med", false)
		way:MinZoom(mz)
		way:Attribute("pmap:kind", kind)
...
function setRoadLabels(way)
    -- Doing street labels here. Original version had a separate
    -- `process-street-labels` function that you could refer to if you want to
    -- try to improve this. I just didn't want to redo the "pmap:kind" logic.
    -- The benefit of the separate `process-street-labels` is that the names
    -- show up at custom nice looking zoom levels which may be nice.
    if way:Holds("name") then
        setNameAttributes(way)
    end
    if way:Holds("ref") then
...
function process_street_polygons(way)
	...
	if mz < inf_zoom then
		...
		setZOrder(way, rail, false)
		if name ~= "" then
			way:LayerAsCentroid("roads_polygons_labels")
			setNameAttributes(way)
			way:Attribute("kind", kind)
			way:MinZoom(mz)
		end
